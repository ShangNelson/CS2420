...
void Collection<Item>:: swap(int first, int second){
    Item secondInstance = elements[second];
    elements[second] = elements[first];
    elements[first] = secondInstance;
}
...
void Collection<Item>::sortByBubble(){
    ...
    while (hasSwapped) { // Cut out all excess passes after fully sorted
        hasSwapped = false;
...
int Collection<Item>::partition(int start, int end){
    int pivot = start;
    Item toComp = elements[start];
    for (int i = start + 1; i <= end; i++) {
        if (elements[i] < toComp) {
            pivot++;
            swap(i, pivot);
        }
    }
...
Item Collection<Item>::findMinInRange(int start){
    Item min = start;
    while (start < curSize) {
        if (elements[start] < elements[min])
            min = start;
        start++;
    }
    return min;
